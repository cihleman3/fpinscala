...
import fpinscala.iomonad.IO3.{freeMonad, runConsoleFunction0}
...
object IO2a {
  ...
  // reassociate to the right as `a flatMap (ar => f(ar) flatMap g)`
  @annotation.tailrec
...
object IO3 {
  ...
  def translate[F[_], G[_], A](f: Free[F, A])(fg: F ~> G): Free[G, A] = {
    type Gp[T] = Free[G, T]
    val t: F ~> Gp = new (F ~> Gp) {
      def apply[B](fa: F[B]): Free[G, B] = {
        val x: G[B] = fg(fa)
        Suspend(x)
      }
    }
    implicit val v: Monad[Gp] = freeMonad[G]
    runFree[F, Gp, A](f)(t)
  }
