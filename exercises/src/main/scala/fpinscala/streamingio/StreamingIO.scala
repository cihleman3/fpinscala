...
import fpinscala.streamingio.SimpleStreamTransducers.Process.lift
...
object SimpleStreamTransducers {
  ...
  sealed trait Process[I, O] {
    ...
    def zipWithIndex: Process[I, (O, Int)] = {
      def go(acc: Int, p: Process[I, O]): Process[I, (O, Int)] = p match {
        case Emit(head, tail) => Emit((head, acc), go(acc + 1, tail))
        case Await(pull) =>
          Await(oi => go(acc, pull(oi)))
        case Halt() => Halt()
      }
      go(0, this)
    }
  ...
  object Process {
     ...
     */
    def take[I](n: Int): Process[I, I] = liftOne(identity[I]).repeatN(n)

    def drop[I](n: Int): Process[I, I] = dropOne.repeatN(n)
    ...
    def mean: Process[Double, Double] = {
      def go(n: Double, acc: Double): Process[Double, Double] =
        await(d => emit((d + acc) / n, go(d + 1, d + acc)))
      go(1.0, 0.0)
    }
     ...
     */
    def exists[I](f: I => Boolean): Process[I, Boolean] = lift(f) |> filter(_ == true)
    ...
    def runLog(implicit F: MonadCatch[F]): F[IndexedSeq[O]] = {
      def go(cur: Process[F, O], acc: IndexedSeq[O]): F[IndexedSeq[O]] =
        cur match {
          ...
          case Await(req, recv) =>
            val att = F.attempt(req)
            F.flatMap(att) { e => go(Try(recv(e)), acc) }
        }
      go(this, IndexedSeq())
    }
